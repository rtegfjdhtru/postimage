...
class Auth_Login_SimpleAuth extends \Auth\Auth_Login_SimpleAuth{
     ...
     * @return  bool
     */
    public function create_user($password, $email, $group = 1, Array $profile_fields = array())
    {
        $password = trim($password);

        /*１カ所目：Email形式かどうかの判定なので、バリデーションクラスでやる場合は不要。削除。
        $email = filter_var(trim($email), FILTER_VALIDATE_EMAIL);
        */

        /*２カ所目：emptyチェックなので、バリデーションクラスでやる場合は不要。削除。
